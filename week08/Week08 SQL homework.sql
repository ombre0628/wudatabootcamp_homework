...
ALTER TABLE actor 
ADD COLUMN actor_description BLOB NULL DEFAULT NULL ;
...
SELECT last_name, Count(*) FROM actor AS number_of_actor 
GROUP BY last_name;
...
SELECT staff.first_name, staff.last_name, SUM(payment.amount) AS total_received
FROM staff
INNER JOIN payment 
...
SELECT film.title, COUNT(film_actor.actor_id) AS number_of_actor
FROM film_actor  
INNER JOIN film
